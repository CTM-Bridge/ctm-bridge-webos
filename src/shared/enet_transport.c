/* enet_transport.c -- see enet_transport.h.
 *
 * Additive ENet/UDP client used by the dual-probe loop. Mirrors the TCP framing
 * in main.c (send_msg/recv_msg) exactly: one CtmBridgeProtocol header followed
 * by its payload, one message per reliable ENet packet on channel 0.
 */
#define _GNU_SOURCE
...
/* One decoded inbound or pending outbound message. */
typedef struct {
    ctmb_header_t header;
    uint8_t *payload; /* malloc'd, may be NULL when payload_len == 0 */
} ctm_enet_msg_t;
...
struct ctm_enet_client {
    ...
    /* Inbox is only touched on the owning (service) thread, but a mutex keeps it
     * uniform and cheap. */
...
ctm_enet_client_t *enet_client_create(void) {
    ...
    /* Client host: no bound address, 1 outgoing connection, single channel. */
    client->host = enet_host_create(NULL, 1, CTM_ENET_CHANNEL_COUNT, 0, 0);
    if (!client->host) {
        free(client);
        return NULL;
    }
...
/* Decode a received packet into the inbox (drops the oldest on overflow). */
static void enet_client_ingest(ctm_enet_client_t *client, const enet_uint8 *data, size_t length) {
    if (!data || length < sizeof(ctmb_header_t)) return;
    ctmb_header_t header;
    memcpy(&header, data, sizeof(header));
    if (header.magic != CTMB_MAGIC || header.version != CTMB_VERSION ||
        header.payload_len > CTMB_MAX_PAYLOAD) {
        return;
    }
    if (length < sizeof(ctmb_header_t) + header.payload_len) return;
...
/* Put one queued outbound message on the wire. Owning-thread only. */
static void enet_client_dispatch(ctm_enet_client_t *client, const ctm_enet_msg_t *msg) {
    ...
    /* Input reports are stateful and high-rate: send them unreliable-sequenced
     * (flag 0) so a dropped UDP datagram is simply superseded by the next report
     * instead of stalling every later report behind a reliable retransmit
     * (head-of-line blocking -- the source of the latency spikes, jitter, and
     * halved rate). Everything else (hello/config/feature/output) stays
     * reliable. ENet sequences reliable and unreliable independently per channel,
     * so sharing CTM_ENET_CHANNEL is fine. */
...
int enet_client_connect(ctm_enet_client_t *client, const char *host, int port, unsigned int timeout_ms) {
    if (!client || !client->host || !host) return -1;
    ...
    for (;;) {
        ...
        if (rc > 0) {
            /* A stray receive/disconnect before connect completes. */
    ...
    /* Timed out or failed: tear the half-open peer down hard. */
    if (client->peer) {
        enet_peer_reset(client->peer);
        client->peer = NULL;
    }
...
void enet_client_disconnect(ctm_enet_client_t *client) {
    ...
    if (client->peer && client->connected) {
        ...
        /* Pump briefly so the disconnect is delivered, then force-reset. */
        uint64_t start = enet_now_us();
        for (;;) {
            ENetEvent event;
            int rc = enet_host_service(client->host, &event, 10);
...
int enet_client_service(ctm_enet_client_t *client, unsigned int timeout_ms) {
    if (!client || !client->host) return -1;
    int dropped = 0;
    ...
    if (rc > 0) {
        ...
        /* Drain any further immediately-available events without blocking. */
    ...
    /* Push anything the input thread queued during this tick. */
    enet_client_flush_outbox(client);
    return dropped ? -1 : 0;
}
...
int enet_client_send_msg(ctm_enet_client_t *client, uint16_t type, uint32_t flags,
    ...
    if (len > CTMB_MAX_PAYLOAD) return -1;
    ...
    if (client->out_count >= CTM_ENET_OUTBOX_CAP) {
        /* Drop oldest to bound memory; reliable channel still delivers the rest
         * in order. */
